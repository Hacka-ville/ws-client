...
const CreateTravelPage = (props) => {
  ...
  let titlePage = !active
    ? "Select the most convenient flight!"
    : active === 1
    ? "Select the hotel you have a taste for!"
    : active === 2
    ? "Select the activities you enjoy the most!"
    : active === 3
    ? "Now let's make sure that everything is as you wish to be!"
    : "Select the most convenient flight!";
    ...
    switch (step) {
      ...
      case 3:
        return (
          <Payments
            elements={[
              { name: "Hotel London", price: "1200", category: "Location" },
            ]}
          />
        );
        break;
  ...
  return (
    <React.Fragment>
      ...
      <Grid
        grow
        gutter="md"
        style={{ minHeight: "calc(100vh - 160px)", padding: "80px" }}
      >
        <Grid.Col span={2}>
          <TimelineComponent active={active} />
        </Grid.Col>
        <Divider orientation="vertical" size="md" />
        <Grid.Col span={9}>{renderStep(active)}</Grid.Col>
      </Grid>
      <Button
        onClick={() => setActive(active - 1)}
        leftIcon={<IconPlayerTrackPrev size={14} />}
        style={{
          position: "absolute",
          left: "100px",
      ...
      <Button
        onClick={() => setActive(active + 1)}
        leftIcon={<IconPlayerTrackNext size={14} />}
        style={{
          position: "absolute",
          right: "100px",
          display: `${active === 3 && "none"}`,
        }}
      >
        Next step
      </Button>
